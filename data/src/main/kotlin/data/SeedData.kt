...
object SeedData {
    private val recommendations =
            listOf(
                    RecommendationItem(
                            ...
                            "Интерстеллар",
                            listOf("sci-fi", "drama"),
                            listOf("thoughtful", "epic"),
                            2014,
                            "Фильм о выборе, времени и дальнем космосе."
                    ),
                    RecommendationItem(
                            2,
                            RecommendationType.FILM,
                            "1+1",
                            listOf("drama", "comedy"),
                            listOf("warm", "uplifting"),
                            2011,
                            "История дружбы, которая поднимает настроение."
                    ),
                    RecommendationItem(
                            3,
                            ...
                            "Тед Лассо",
                            listOf("comedy", "drama"),
                            listOf("uplifting", "kind"),
                            2020,
                            "Лёгкий и добрый сериал про команду и характер."
                    ),
                    RecommendationItem(
                            4,
                            RecommendationType.SERIES,
                            "Очень странные дела",
                            listOf("sci-fi", "mystery"),
                            listOf("tense", "nostalgic"),
                            2016,
                            "Таинственная история с атмосферой 80-х."
                    ),
                    RecommendationItem(
                            5,
                            RecommendationType.BOOK,
                            "451° по Фаренгейту",
                            listOf("dystopia", "classic"),
                            listOf("thoughtful", "dark"),
                            1953,
                            "Классика, которая отлично подходит для вдумчивого вечера."
                    ),
                    RecommendationItem(
                            6,
                            RecommendationType.BOOK,
                            ...
                            listOf("cozy", "magical"),
                            1997,
                            "Погружение в волшебный мир для любого возраста."
                    ),
                    RecommendationItem(
                            7,
                            ...
                            listOf("calm", "cozy"),
                            2016,
                            "Спокойная игра для отдыха и маленьких достижений."
                    ),
                    RecommendationItem(
                            8,
                            RecommendationType.GAME,
                            "Hades",
                            listOf("roguelike", "action"),
                            listOf("energetic", "challenging"),
                            2020,
                            "Быстрый экшен с сильной атмосферой и прогрессом."
                    ),
                    RecommendationItem(
                            9,
                            RecommendationType.FILM,
                            ...
                            listOf("cozy", "light"),
                            2001,
                            "Тёплая история для хорошего настроения."
                    ),
                    RecommendationItem(
                            10,
                            RecommendationType.SERIES,
                            ...
                            listOf("serious", "intense"),
                            2019,
                            "Короткий сильный сериал с высокой концентрацией событий."
                    )
            )

    private val defaultsPredictions =
            listOf(
    ...
    private val defaultsTests =
            listOf(
                    Triple(
                            "Мини-тест: логика",
                            QuestionKind.SINGLE,
                            "Сколько будет 7 + 5? | 10;11;12;13 | 3"
                    ),
                    Triple(
                            "Мини-тест: логика",
                            QuestionKind.SINGLE,
                            "Продолжи последовательность: 2, 4, 8, ? | 10;12;14;16 | 4"
                    ),
                    Triple(
                            "Мини-тест: настроение",
                            QuestionKind.NUMBER,
                            "Сколько минут нужно на короткий перерыв? | 5"
                    ),
                    Triple(
                            "Мини-тест: ассоциации",
                            QuestionKind.MATCH,
                            "Соедини пары | кот=мяу;собака=гав"
                    )
            )
    ...
    fun ensure() {
        ...
        try {
            val mapping =
                    mapOf(1001L to "avatar_1.jpg", 1002L to "avatar_2.jpg", 1003L to "avatar_3.jpg")

            mapping.forEach { (userId, avatarFileName) ->
                val classpathPath = "mocks/avatars/$avatarFileName"

                val devCandidates =
                        listOf(
                                File(
                                        "data${File.separator}src${File.separator}main${File.separator}resources${File.separator}$classpathPath"
                                ),
                                File(
                                        "data${File.separator}build${File.separator}resources${File.separator}main${File.separator}$classpathPath"
                                ),
                                File(
                                        ".${File.separator}data${File.separator}src${File.separator}main${File.separator}resources${File.separator}$classpathPath"
                                )
                        )

                val existingFile = devCandidates.firstOrNull { it.exists() }
                if (existingFile != null) {
                    mockAvatarPaths[userId] = existingFile.absolutePath
                } else {
                    mockAvatarPaths[userId] = classpathPath
                }
            }
        } catch (e: Exception) {
        ...
        if (Db.single("select id from tests limit 1", map = { it.getLong("id") }) == null) {
            defaultsTests.forEach { (title, kind, raw) ->
                val testId =
                        Db.insertId("insert into tests(title, author_id) values (?, null)") { stmt
                            ->
        ...
        mockUsers.forEach { user ->
            ...
            if (existingUser == null) {
                ...
                if (newAvatarFileId != null &&
                                (currentAvatarFileId == null ||
                                        currentAvatarFileId != newAvatarFileId)
                ) {
                    Db.execute("update users set avatar_file_id = ? where user_id = ?") { stmt ->
                        stmt.setString(1, newAvatarFileId)
                        stmt.setLong(2, user.userId)
                    }
                    println("✅ Обновлена аватарка для пользователя $userId: $newAvatarFileId")
                } else if (newAvatarFileId == null && user.userId !in setOf(1001L, 1002L, 1003L)) {
                    // На остальных мок пользователей аватарки не ставим. Если ранее уже была
                    // выставлена мок-аватарка — сбрасываем.
                    val hasMockAvatar =
                            currentAvatarFileId?.contains("mocks/avatars/avatar_") == true ||
                                    currentAvatarFileId?.contains("avatar_1.jpg") == true ||
                                    currentAvatarFileId?.contains("avatar_2.jpg") == true ||
                                    currentAvatarFileId?.contains("avatar_3.jpg") == true

                    if (hasMockAvatar) {
                        Db.execute("update users set avatar_file_id = null where user_id = ?") {
                                stmt ->
                            stmt.setLong(1, user.userId)
                        }
                        println("✅ Сброшена аватарка для пользователя $userId")
                    }
                } else {
                    if (newAvatarFileId == null) {
                        println("⚠️ Новая аватарка не найдена для пользователя $userId")
                    } else {
                        println("ℹ️ Аватарка для пользователя $userId уже актуальна")
                    }
                }
            }
            ...
            if (user.isBanned) {
                Db.execute(
                        """insert into banned_users(user_id, banned_at, reason) values (?, ?, ?) on conflict (user_id) do nothing"""
                ) { stmt ->
                    stmt.setLong(1, user.userId)
